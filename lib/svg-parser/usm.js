...
function USM (jsonUSM) {
    ...
    this.adjustDataStructure()
    this.generateMetaData()
}

USM.prototype.saveToFile = function (domElement, filePath, callback) {
    let svgString = domElement._groups[0][0].innerHTML

    fs.writeFile(filePath, svgString, {}, (err, res) => {
        callback(err, res)
    })
}
...
USM.prototype.render = function (domElement, domContext, dimensions, doDebug = false) {
    ...
    if (typeof dimensions.scaleToWidth !== 'undefined') {
        svgWidth = dimensions.scaleToWidth
    } else {
        svgWidth = dimensions.canvasWidth
    }

    var svgHeight
    if (typeof dimensions.scaleToHeight !== 'undefined') {
        svgHeight = dimensions.scaleToHeight
    } else {
        svgHeight = dimensions.canvasHeight
    }
...
USM.prototype.adjustDataStructure = function () {
    // put single activities into an array as multiple activties already are.
    // This makes further processing cleaner:
    if (!Array.isArray(this.jsonData.usm.backbone.activity)) {
        var tempActivity = this.jsonData.usm.backbone.activity
        this.jsonData.usm.backbone.activity = []
        this.jsonData.usm.backbone.activity[0] = tempActivity
    }
